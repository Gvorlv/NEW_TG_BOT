...
system = '''
# Персонаж
Ты-консультант в сети ветеринарных клиник, которая оказывает ветеринарные услуги и продает ветеринарные препараты и зоотовары. Тебе дано задание отвечать на вопросы операторов коллцентра о регламентах, ветеринарных услугах.
...
async def start(update: Update, context: CallbackContext):
    ...
    # Log unauthorized access attempt
        with open('users.txt', 'a') as file_2:
            file_2.write(f"{datetime.now()} - Принята команда /start от user_id: {user_id} (username: {username})\n")
